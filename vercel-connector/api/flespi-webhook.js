...
async function persistEvent({ firestore, config, event, classification }) {
  ...
  if (classification.vibrationAlarm || classification.geofenceAlarm) {
    ...
    if (classification.geofenceAlarm) {
      // NO ejecutar el filtrado anti-fantasma durante operaciones explícitas de configuración
      // de geocercas (create/update/delete). Estas operaciones son conscientes del usuario
      // y deben propagarse aunque existan recálculos.
      if (classification.geofenceConfigChange) {
        writeLog('info', 'skip anti-fantasma during geofence config change', { device: event.deviceId });
      } else {
        try {
          const raw = event.raw || {};
